...
export default async function RecentArticleSingleLineListView() {
    ...
    const articleCollection = await contentfulClient.getEntries<TypeBlogSkeleton>({
      ...
      const andmoreCount = articleCollection.items.length - MAX_SHOW_COUNT > 0 ? articleCollection.items.length - MAX_SHOW_COUNT : 0;

    return (
        <ArticleSingleLineListView articles={articles} andmoreCount={andmoreCount} />
    )
}